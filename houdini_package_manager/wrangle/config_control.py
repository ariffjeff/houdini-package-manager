...
class PackageCollection:
    ...
    # def refresh_only_missing_package_repo_data_file(self) -> None:
    #     """
    #     Load the local packages' git repo data for this Houdini version from the local json file.

    #     Missing data is fetched from remote repos and written to the json file.
    #     Does not refresh outdated data.
    #     """

    #     # init packages' local repo json file if doesn't exist.
    #     # if it does, load the data.
    #     if not self.PACKAGES_GIT_DATA_PATH.exists():
    #         with open(self.PACKAGES_GIT_DATA_PATH, "w") as file:
    #             json.dump({}, file, indent=4)
    #         json_local = {}
    #     else:
    #         json_local = self.get_stored_repo_metadata()

    #     # check if this houdini version's packages' repo metadata is stored in the local user metadata json file.
    #     # if it isn't, fetch the remote data, then append it to the json file.
    #     json_remote = {}
    #     if self.houdini_version not in json_local:
    #         logging.debug(
    #             f"No repo data for {self.houdini_version} found in {self.PACKAGES_GIT_DATA_PATH}. Fetching and writing remote data..."
    #         )
    #         json_remote = self.get_remote_repo_metadata()
    #         json_local.update(json_remote)
    #         # this file-written json will be read back in soon if there are more houdini
    #         # versions (that have packages) so that the json can get appended to.
    #         with open(self.PACKAGES_GIT_DATA_PATH, "w") as file:
    #             json.dump(json_local, file, indent=4)
    #     else:
    #         logging.debug(
    #             f"Repo data for {self.houdini_version} found in {self.PACKAGES_GIT_DATA_PATH}. Not updating data."
    #         )

    # def get_stored_repo_metadata(self) -> dict:
    #     """
    #     Load local pre-stored packages' repo metadata (previously fetched from the GitHub API), if it exists.
    #     """

    #     if not self.PACKAGES_GIT_DATA_PATH.exists():
    #         return {}

    #     with open(self.PACKAGES_GIT_DATA_PATH, "r") as file:
    #         data = json.load(file)
    #     return data

    # def get_remote_repo_metadata(self) -> dict:
    #     """
    ...
    #         self.pkgs[name]._repo.get_remote_data()
    #         # repo = pkg._repo
    ...
    #             "latest_version": self.pkgs[name]._repo.remote.tag_latest,
    #         }
