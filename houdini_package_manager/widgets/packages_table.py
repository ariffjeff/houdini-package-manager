...
class CellWidgets:
    ...
    def button_warning(parent: PackageTableModel, row: int, warnings: str, config_path: Path) -> SvgPushButton:
        ...
        if pkg_name[-5:] != ".json":
            pkg_name += ".json"
        button_warning.set_hover_status_message(
            f"Can't process package, error(s) in config: {pkg_name}. JSON might have trailing commas or other syntax"
            " errors. Fix errors then refresh this table."
        )
